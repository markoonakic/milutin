...
2. List all notes with full directory tree using `Bash` find:
   ```bash
   find $VAULT_BASE_PATH -name "*.md" -type f | head -50
   ```
3. Use `Glob` to explore directory structure:
   ...
   - Published content: `Output/**/*.md`
4. Read key hub notes from major clusters using `Read` tool (Systems Thinking, Mental Models, Decision-Making, Learning, etc.)
