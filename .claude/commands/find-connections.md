...
1. If given a note name, use `Grep` to find files matching the name:
   ```
   grep -r "# $ARGUMENTS" $VAULT_BASE_PATH --include="*.md"
   ```
...
1. Use `mcp__smart-connections__get_similar_notes` on the anchor note:
   - Note path: Use full vault-relative path returned from search (e.g., `Zettelkasten/Your-Note.md`)
   - Limit: 10 results
   - Threshold: 0.6
...
3. Look for:
   ...
   - Meta-patterns (e.g., "state management" appearing in Kubernetes, React, databases)
