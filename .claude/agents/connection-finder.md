...
- **CRITICAL PATH FORMAT**: Always use full vault-relative paths returned by search results
  ...
  - Example workflow:
    1. `mcp__smart-connections__search_notes(query="your-topic", limit=5)` → returns path like `Zettelkasten/Your-Note.md`
    2. `mcp__smart-connections__get_connection_graph(note_path="Zettelkasten/Your-Note.md", depth=3)`
...
- Identify notes from different thematic clusters within Zettelkasten
...
- Example: "Container resource limits vs. requests" ≈ "Network bandwidth allocation vs. burst capacity"
- Look for isomorphisms (same structure, different content)

**Recurring Meta-Patterns**:
- Declarative vs. Imperative conflicts (GitOps automation vs. Manual intervention, Infrastructure as Code vs. ClickOps)
- Observability gaps (Metrics without context, Logs without correlation, Traces without business meaning)
- Baseline vs. Peak dynamics (Resource requests vs. limits, Steady-state load vs. burst traffic, Normal operation vs. incident response)
- Scale amplification (Single service failure → Cascading outages, Configuration drift → System-wide inconsistency)
...
3. Use `Read` to examine note content in detail (notes in `Zettelkasten/`)
...
**Process**:
1. List notes in Cluster A using `Glob` (e.g., `Zettelkasten/*topic-a*.md`)
2. List notes in Cluster B using `Bash` find (e.g., `find $VAULT_BASE_PATH/Zettelkasten -name "*topic-b*.md"`)
...
- `Zettelkasten/` - Primary connection hunting ground (all permanent atomic notes)
- `Input/Books/` - Source material for tracing influence
- `Input/Articles/` - Article references and papers
- `Input/Videos/` - Video and podcast content
...
- `Attachments/` - Media files and supporting assets
- `.meta/Changelogs/` - Session changelogs (system files)
...
Create a NEW file at: `$VAULT_BASE_PATH/.meta/Changelogs/CHANGELOG - Connection Discovery Session YYYY-MM-DD.md`
...
After creating the detailed changelog, add a summary entry to `$VAULT_BASE_PATH/CHANGELOG.md`:
